...
namespace EventHubsProcessor_BasicCheckpointing
{
    class Program
    {
        ...
        static async Task Main(string[] args)
        {
            ...
            var secretName = "sas-listener-evh-shorturl2-primary-connection-string";
            ...
            ConnectionStringEventHub = secret.Value;

            Console.Write("Blob Connection String: ");
//            ConnectionStringBlob = Console.ReadLine();
            secretName = "stazfuncshorturl2-primary-connection-string";
            secret = await keyVaultClient.GetSecretAsync($"{keyVaultUrlBaseUrl}secrets/{secretName}").ConfigureAwait(false);
            ConnectionStringBlob = secret.Value;
        ...
        static async Task<string> GetHelloWorldAsync()
        {
            ...
            BlobContainerClient storageClient = new BlobContainerClient(ConnectionStringBlob, "ehub-checkpoint");
            EventProcessorClient processor = new EventProcessorClient(storageClient, consumerGroup, ConnectionStringEventHub, "evh-shorturl2");
